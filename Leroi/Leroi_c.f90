...
! Changes
!
!   8.0.4a: adds displacement current correction
!
!	8.0.4
...
!           I = 1,2,3 = north, east and vertical componsnts respectively
 ...
 COMPLEX(KIND=QL) SIGL(NLYR),KSQL(NLYR),HLYR(6),BZZ,BXX,BZX,BXZ,BYX,BYZ,BX,BY,BZ,BZZD,BXXD, &
                  BZXD,BXZD,BYXD,BYZD,BFDD(NFRQ,MRXTX,NTX,3),DISPC
 COMPLEX BFD(NFRQ,MRXTX,NTX,3)
 Logical :: WRITE_FRQ = .True.
...
!  the system by SXAZM
 ...
 DO JF = 1,NFRQ
   ...
   CALL HSMDB_HNK (KFG,SXLYR,RXLYR,NLYR,THKD,DPTHL,SIGL,KSQL,RMUD,ZS,ZR,RHOD,NINTG,HLYR,DISPC)
...
! magnetic fields in a whole space with the geoelectric properties of layer RXLYR.
 ...
 SUBROUTINE HSMDB_HNK (KFG,SXLYR,RXLYR,NLYR,THKD,DPTHL,SIGL,KSQL,RMUD,ZS,ZR,RHOD,NINTG,HLYR,DISPC)
!-------------------------------------------------------------------------------------------
...
!    OUTPUT is HLYR(1:NINTG)  forward model components
 ...
 COMPLEX(KIND=QL) SIGL(NLYR),KSQL(NLYR),HLYR(NINTG),DISPC
 LOGICAL JUMP
 ...
 DO L = -50, JNHI             ! Start at L = -50 to pick up low values.
   ...
   LMBDA = EXP (Y)
   CALL HSMDB_KER (L,LMBDA,NLYR,THKD,DPTHL,SIGL,KSQL,RMUD,KFG,RXLYR,SXLYR,ZS,ZR,RHOD,NINTG,HLYR,JUMP,DISPC)
 ...
 SUBROUTINE HSMDB_KER (L,LMBDA,NLYR,THKD,DPTHL,SIGL,KSQL,RMUD,KFG,RXLYR,SXLYR,ZS,ZR,RHOD,NINTG,HLYR,JUMP,DISPC)
!--------------------------------------------------------------------------------------------------------
...
!  time-domain responses
 ...
 COMPLEX(KIND=QL) XI_V,XI_H,F_V,ETA_V,ETA_H,G_V,QFD(5),XP1,SL,SM,KSQL(NLYR), &
                  SIGL(NLYR),S(0:NLYR),FW(NINTG),HLYR(NINTG),DISPC
 LOGICAL JUMP

 CALL MDSX_COEF (SXLYR,RXLYR,KFG,LMBDA,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL, &
                 ZS,S,XI_V,XI_H,F_V,ETA_V,ETA_H,G_V,DISPC)
...
!                               where ZR = receiver depth relative to surface
 ...
 COMPLEX(KIND=QL) SM,SL,VLG,VMG,AMG,VLF,VMF,AMF,XPA,XP1,XP2,XQ1,XQ2,DENOMF,DENOMG, &
                  ...
                  F_VBAR,ETA_VBAR,ETA_HBAR,G_VBAR,LMBSQ,VACHI,DISPC
 COMPLEX(KIND=QL), DIMENSION (0:NLYR) :: S,T,R,AF,AG,VF,VG,CHI
 COMPLEX(KIND=QL), DIMENSION (NLYR) :: SIGL,KSQL
...
! T & R are the magnetic and magnetic reflection coefficients
 ...
 LMBSQ = CMPLX (LMBDA * LMBDA, 0._QL, KIND=QL)
 S(0) = CMPLX (LMBDA, 0._QL,KIND=QL)
 S(0) = SQRT (LMBSQ - DISPC)			! to match LeroiAir ...
 DO J = 1,NLYR
   S(J) = SQRT (KSQL(J) + LMBSQ)
   RMUSQ(J) = RMUD(J) * RMUD(J)
 END DO
...
! due to exponentials.
 ...
 CALL MDSX_COEF (SXLYR,RXLYR,KFG,LMBDA,NLYR,THKD,DPTHL,RMUD,SIGL,KSQL, &
                 ZS,S,XI_V,XI_H,F_V,ETA_V,ETA_H,G_V,CMPLX(0, 0, QL))
 SL = S(RXLYR)
 SM = S(SXLYR)
 LMBDA2 = LMBDA**2
